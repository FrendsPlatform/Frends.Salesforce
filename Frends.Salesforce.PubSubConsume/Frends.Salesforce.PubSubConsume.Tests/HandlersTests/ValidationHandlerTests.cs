...
public class ValidationHandlerTests
{
    ...
    public void BasicValidationShouldPass()
    {
        ...
        Assert.DoesNotThrow((System.Action)(() => ValidationHandler.Run(foobar)));
    }

    [Test]
    public void ValidationShouldFailOnNullObject()
    {
        var ex = Assert.Throws<ValidationException>((System.Action)(() => ValidationHandler.Run([null])));
    ...
    {
        var ex = Assert.Throws<ValidationException>((System.Action)(() => ValidationHandler.Run()));
        Assert.That(ex, Is.Not.Null);
    ...
    {
        TestClass foobar = new();
        var ex = Assert.Throws<ValidationException>((System.Action)(() => ValidationHandler.Run(foobar, null)));
        Assert.That(ex, Is.Not.Null);
        Assert.That(ex.Message, Contains.Substring("Validated object can't be null!"));
